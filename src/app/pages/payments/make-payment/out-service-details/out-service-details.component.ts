...
@Component({
  selector: 'app-out-service-details',
    templateUrl: './out-service-details.component.html',
    styleUrls: ['./out-service-details.component.scss']
})
export class OutServiceDetailsComponent implements OnInit {
  ...
  load(): void {
    ...
    .retryWhen((err) => {
      ...
      (data: any) => {
        ...
        let third: any[] = this.paymentsServices.paymentType.OutstandingServiceDebtHistory;
        this.OutstandingFineDebtHistory = third;
        
        const sum3 = new List<any>(third).Sum(x => x.amount);
      
        
        this.rows = this.paymentsServices.paymentType.OutstandingServiceDebtHistory;
