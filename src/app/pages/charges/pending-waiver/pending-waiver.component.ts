...
@Component({
  selector: 'app-pending-waiver',
  templateUrl: './pending-waiver.component.html',
  styleUrls: ['./pending-waiver.component.scss']
})
export class PendingWaiverComponent implements OnInit {
...
columnId: number;
 ...
 //   this.ItemsPerPage = this._GeneralService.ItemsPerPage;
  ...
  action(row, status) {
    Swal({
      ...
      if (result.value) 
      {
        ...
        let url = `auth/approve-waiver-request?token=` + token; 
  ...
  load(): void {
    ...
    this._GeneralService.getT(token, 'auth/pending-fine-waivers?token=')
...
//{ path: 'apply-waiver', component: ApplyWaiverComponent, data: { breadcrumb: 'Apply Waiver' } },
  ...
  addServiceCharge(row: any, chargeType: any) {
