...
import { CounterChqReqDetailsComponent } from '../counter-chq-req-details/counter-chq-req-details.component';
...
@Component({
  selector: 'app-counter-chq-req-list',
  templateUrl: './counter-chq-req-list.component.html',
  styleUrls: ['./counter-chq-req-list.component.scss']
})
export class CounterChqReqListComponent implements OnInit {
  ...
  rows: CounterChequeForm[];
  rowSearch: InstrumentForm[];

  counterChequeBond: CounterChequeForm;
  currentDate = '';
  accttypes = [];
  currencies = [];
  columns = [];
  temp:CounterChequeForm[];
  current_page = 1;
...
tab3 = ''
  ...
  {
     //console.log()
      this.counterChequeBond = new CounterChequeForm();
...
searchHistory()
{
  ...
  let val = 
        {
          transactionDate: this.counterChequeBond.transactionDate,
          psAcctNo: this.counterChequeBond.acctNo,
          accountName: this.counterChequeBond.acctName,
          psAcctType: this.counterChequeBond.acctType,
          //pnGlobalView: 'N',
          ServiceId: this.serviceId,
          RoleId: this.assignRole.roleId,
          MenuId:  this.menuId,
          referenceNo : this.counterChequeBond.referenceNo,
          serviceStatus: this.counterChequeBond.serviceStatus,
          acctStatus: this.counterChequeBond.status,
          
          psCcyCode: this.counterChequeBond.ccyCode,
          pnDeptId: this.counterChequeBond.processingDeptId,
          psBranchNo: this.counterChequeBond.branchNo
        }
        //this.assignRole.roleId
    console.log('Counter Cheque value param search', val);

    let url = 'CounterCheque/GetAllSearchHistory';

    this._GeneralService.post(val, url)
    .subscribe((response: any) => {
      console.log("response Counter search " + response._response);
...
clearItem()
{
  this.counterChequeBond.transactionDate = "";
  this.counterChequeBond.ccyCode = '';
  this.counterChequeBond.status = '';
  this.counterChequeBond.acctName = '';
  this.counterChequeBond.acctNo = '';
  this.counterChequeBond.acctStatus = '';
  this.counterChequeBond.acctType = '';
  this.counterChequeBond.processingDeptId = '',
  this.counterChequeBond.branchNo = '',
  this.counterChequeBond.availBal = '',
  this.counterChequeBond.referenceNo = ''
}
...
fromDate(events)
{
    var dateCon = this._GeneralService.dateconvertion(events.target.value);
      ...
      this.counterChequeBond.transactionDate = dateCon;
  ...
  View(action: any, record: any, createdBy: any): void
   {
    let dialogRef = this.dialog.open(CounterChqReqDetailsComponent, {
      ...
      data: { actionName: action, record:  record, createdBy: createdBy, 
        ...
        serviceName: 'Counter Cheque',  admService :  this.admService
      },
