...
import { ChqSerializationDetailsComponent } from '../chq-serialization-details/chq-serialization-details.component';
...
@Component({
  selector: 'app-chq-serialization-list',
  templateUrl: './chq-serialization-list.component.html',
  styleUrls: ['./chq-serialization-list.component.scss']
})
export class ChqSerializationListComponent implements OnInit {
...
tab2 = '';
  ...
  ngOnInit() 
  {
   ...
   load(param: any): void {
    ...
    let track = 0;
    // let url = 'ChqBookReqEncoding/GetAll';
    let url = 'ChqBookSerialize/GetAll';
      let val = 
        {
  ...
  this._GeneralService.homePage(val, url)
    ...
    (data: any) => 
    {
        ...
        if(recordData != null)
        {
          this.View(actionData, recordData, data.fullName);
        }
...
serialize(): void {
  ...
  if(getSelected.length === 0) {
    ...
    Swal({
      ...
      title: '',
      text: 'Are you Sure you want to Serialize the Selected Cheque Request(s)?',
      type: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Yes',
      confirmButtonColor: this.btnConfirm, 
      ...
      if (result.value) 
      {
        ...
        this.loadPage = true;
        this.actionLoaderUpdate = true;
        let element = <HTMLInputElement> document.getElementById("btnSerializeChq");
        
        element.disabled = true;
        //console.log('this.selectedRec: ', this.selectedRec);
      
        let url = 'ChqBookSerialize/AddChqRange';
        ...
        let val = 
        {
          ListOprChqBookRequest: getSelected,
          loginUserName: userDetails.userName,
          userId: userDetails.userId,
        }
