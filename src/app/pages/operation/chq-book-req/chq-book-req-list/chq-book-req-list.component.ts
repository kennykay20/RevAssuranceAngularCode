...
import { ChqBookReqDetailsComponent } from '../chq-book-req-details/chq-book-req-details.component';
...
@Component({
  selector: 'app-chq-book-req-list',
  templateUrl: './chq-book-req-list.component.html',
  styleUrls: ['./chq-book-req-list.component.scss']
})
export class ChqBookReqListComponent implements OnInit {
  ...
  branches = [];
  statusItem = [];
  rowSearch: ChqBookReqForm[];

  chequeBookRequest: ChqBookReqForm;
  ...
  {
     //console.log()
      this.chequeBookRequest = new ChqBookReqForm();
...
load(param: any): void {
    ...
    let track = 0;
    let url = 'ChequeBookRequest/GetAll';
      ...
      let val = 
        ...
        console.log('Cheque param load', val);
  ...
  .retryWhen((err) => {
    ...
    (data: any) => 
    {
      console.log('data checkBokReqList: ', data);
      this.loadPage = false;
...
fromDate(events)
{
  ...
  this.chequeBookRequest.transactionDate = dateCon;
...
let val = 
      {
        transactionDate: this.chequeBookRequest.transactionDate,
        psAcctNo: this.chequeBookRequest.acctNo,
        accountName: this.chequeBookRequest.acctName,
        psAcctType: this.chequeBookRequest.acctType,
        pnGlobalView: 'N',
        RoleId: this.assignRole.roleId,
        MenuId:  this.menuId,
        psStatus: this.chequeBookRequest.status,
        referenceNo : this.chequeBookRequest.referenceNo,
        psBranchNo: this.chequeBookRequest.branchNo,
        serviceId: this.serviceId,
        psCcyCode: this.chequeBookRequest.ccyCode,
        pnDeptId: this.chequeBookRequest.processingDeptId
      }
      //this.assignRole.roleId
  console.log('Cheque Request param search', val);

  let url = 'ChequeBookRequest/GetAllChequeBookSearchHistory';

  this._GeneralService.post(val, url)
  .subscribe((response: any) => {
    //console.log("Cheque request search " + response._response);
    //console.log("Response " + Array.of(response));
...
clearItem(){
  this.chequeBookRequest.transactionDate = '';
  this.chequeBookRequest.acctName = "";
  this.chequeBookRequest.acctNo = '';
  this.chequeBookRequest.acctType = '';
  this.chequeBookRequest.processingDeptId = null;
  this.chequeBookRequest.ccyCode = '';
  this.chequeBookRequest.branchNo = '';
  this.chequeBookRequest.status = '';
  this.chequeBookRequest.referenceNo = '';
}
...
nextPage()
{
  ...
  View(action: any, record: any, createdBy: any): void
   {
    let dialogRef = this.dialog.open(ChqBookReqDetailsComponent, {
      ...
      data: { actionName: action, record:  record, createdBy: createdBy, 
        ...
        serviceName: 'Cheque Book',  admService :  this.admService,
        itbId: record.itbId
      },
  ...
  openDialog(action: any, record: any): any {
    ...
    // let val = 
    // {
    //   oprChqBookRequest: this.instrumentForm.itbId
    // }
