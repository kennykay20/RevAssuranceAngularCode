...
@Component({
  selector: 'app-chq-book-req-details',
  templateUrl: './chq-book-req-details.component.html',
  styleUrls: ['./chq-book-req-details.component.scss']
})

export class ChqBookReqDetailsComponent implements OnInit {
  ...
  constructor(public appSettings: AppSettings,
    ...
    public dialogRef: MatDialogRef<ChqBookReqDetailsComponent>,
    ...
    if (this.data.actionName === 'Add') {
      ...
      //console.log('data this.rowsApprove Chq Bok Request: ', this.rowsApprove);
      this.getById(data.record.itbId);
  ...
  checkLength(value: any, acctType: any, currencyCode: any)
 {
  ...
  getById(recordId): void {
    ...
    console.log('get rec ChequeBookRequest', recordId);
  ...
  update(): void {
    ...
    element.disabled = true;
    let url = 'ChequeBookRequest/Update';
  ...
  initiate(): void {
    ...
    this._GeneralService.post(values, url).subscribe(
      (data: any) => {
        ...
        for (let i = 0; i <= this.chargeFormList.length; i++) {
          if (this.chargeFormList[i] !== undefined) {
            console.log("this.chargeFormList[i]", this.chargeFormList[i]);
            this.chargeFormList[i].transactionDate = this.userDetails.bankingDate;
            this.chargeFormList[i].valueDate = this.userDetails.bankingDate;
        ...
        if (this.instrumentForm.chqProductCode !== null) {
          let lastCheckNo = parseInt(data.instrumentAcctDetails.nLastChqNo) + 1;
          this.instrumentForm.startNo = lastCheckNo.toString();
          ...
          console.log('getNumberOfLeave 33: ', parseInt(getNumberOfLeave[0].chqProductCode));

          if (this.instrumentForm.quantity == 0 || this.instrumentForm.quantity < 0) {
            this.instrumentForm.quantity = 1;
          }

          let value = parseInt(getNumberOfLeave[0].chqProductCode) * this.instrumentForm.quantity;
          //this.instrumentForm.endNo = .val(parseInt(data.nLastChqNo) + parseInt(value))
          console.log("get value ", value);
          let endNoVal = parseInt(data.instrumentAcctDetails.nLastChqNo) + value;
          console.log("get endNoVal ", endNoVal);
          this.instrumentForm.endNo = endNoVal.toString();
          console.log("data.instrumentAcctDetails.nLastChqNo ", data.instrumentAcctDetails.nLastChqNo);
  ...
  validationAcctToPost(serviceId): any {
    ...
    if (this.instrumentForm.collectionBranch == null || this.instrumentForm.collectionBranch == "") {
      swal('', 'Collection Branch is required', 'error')
      return true;
    }
  }
  ...
  chqProductSelected(templateId) {
    ...
    this.instrumentForm.vendorId = getVen.vendorId;

    console.log('chqProductSelected Ven Id: ', this.instrumentForm.vendorId);

    let num = Number(templateId);
    console.log("num", num);
    console.log("this.chargeFormListTemporal ", this.chargeFormListTemporal);
    let get = new List<ChargeForm>(this.chargeFormListTemporal).Where(c => c.templateId === num).ToArray();
    console.log("get charg ", get);
    console.log("this.chargeFormList bef ", this.chargeFormList);
    this.chargeFormList = get;
    console.log("this.chargeFormList aft ", this.chargeFormList);
    console.log("this.chargeFormList aft length ", this.chargeFormList.length);
    for (let i = 0; i < this.chargeFormList.length; i++) {
