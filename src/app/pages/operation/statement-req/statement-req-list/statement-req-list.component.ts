...
import { StatementReqDetailsComponent } from '../statement-req-details/statement-req-details.component';
...
@Component({
  selector: 'app-statement-req-list',
  templateUrl: './statement-req-list.component.html',
  styleUrls: ['./statement-req-list.component.scss']
})
export class StatementReqListComponent implements OnInit {
  ...
  rows: OprAcctStatment[];
  rowSearch: OprAcctStatment[];
  columns = [];
  temp: OprAcctStatment[];
  current_page = 1;
  ...
  currentDate = '';
  acttypes =[];
  statementResearchBound: OprAcctStatment;
...
admStatementChg: admStatementChg[];
  ...
  {
     //console.log()
      this.statementResearchBound = new OprAcctStatment();
   ...
   load(param: any): void {
    ...
    let track = 0;
    let url = 'AccountStatement/GetAll';
  ...
  .subscribe(
    (data: any) => 
    {
      ...
      console.log('data statement this.chargeSetup: ', this.chargeSetup);
...
fromDate(events)
{
  var dateCon = this._GeneralService.dateconvertion(events.target.value);
    ...
    this.statementResearchBound.transactionDate = dateCon;
...
searchHistory()
{
  ...
  let val = 
        {
          transactionDate: this.statementResearchBound.transactionDate,
          psAcctNo: this.statementResearchBound.acctNo,
          accountName: this.statementResearchBound.acctName,
          psAcctType: this.statementResearchBound.acctType,
          //pnGlobalView: 'N',
          ServiceId: this.serviceId,
          RoleId: this.assignRole.roleId,
          MenuId:  this.menuId,
          psStatus: this.statementResearchBound.acctStatus,
          referenceNo : this.statementResearchBound.referenceNo,
          // availBal: this.statementResearchBound.availBal,
          psCcyCode: this.statementResearchBound.ccyCode,
          pnDeptId: this.statementResearchBound.processingDeptId,
          psBranchNo: this.statementResearchBound.branchNo
        }
        //this.assignRole.roleId
    console.log('Account statement value param search', val);

    let url = 'AccountStatement/GetAllSearchHistory';
    ...
    .subscribe((response: any) => {
      ...
      console.log("response statement search " + response._response);
...
clearItem()
{
  this.statementResearchBound.transactionDate = "";
  this.statementResearchBound.ccyCode = '';
  this.statementResearchBound.status = '';
  this.statementResearchBound.acctName = '';
  this.statementResearchBound.acctNo = '';
  this.statementResearchBound.acctStatus = '';
  this.statementResearchBound.acctType = '';
  this.statementResearchBound.processingDeptId = '',
  this.statementResearchBound.branchNo = '',
  // this.statementResearchBound.availBal = '',
  this.statementResearchBound.referenceNo = ''
}
  ...
  View(action: any, record: any, createdBy: any): void
   {
    ...
    let dialogRef = this.dialog.open(StatementReqDetailsComponent, {
      ...
      data: { actionName: action, record:  record, createdBy: createdBy, 
              ...
              serviceName: 'STATEMENT REQUEST DETAILS',  admService :  this.admService,
              admStatementChg: this.admStatementChg
            },
