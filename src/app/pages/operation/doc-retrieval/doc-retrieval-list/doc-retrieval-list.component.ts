...
import { DocRetrievalDetailsComponent } from '../doc-retrieval-details/doc-retrieval-details.component';
...
@Component({
  selector: 'app-doc-retrieval-list',
  templateUrl: './doc-retrieval-list.component.html',
  styleUrls: ['./doc-retrieval-list.component.scss']
})
export class DocRetrievalListComponent implements OnInit {
...
tab2 = '';
  ...
  serviceId = 16;
  documentRetrieve: InstrumentForm;
  ...
  ngOnInit() 
  {
      ...
      this.documentRetrieve = new InstrumentForm();
   ...
   load(param: any): void {
    ...
    let track = 0;
    let url = 'DocumentRetrieval/GetAll';
  ...
  View(action: any, record: any, createdBy: any): void
   {
    let dialogRef = this.dialog.open(DocRetrievalDetailsComponent, {
      ...
      data: { actionName: action, record:  record, createdBy: createdBy, 
        ...
        serviceName: 'Document Retrieval',  admService :  this.admService,
        admDocumentChg: this.admDocumentChg
      },
...
fromDate(events)
{
  var dateCon = this._GeneralService.dateconvertion(events.target.value);
    ...
    this.documentRetrieve.transactionDate = dateCon;
...
searchHistory()
{
  ...
  let val = 
        {
          transactionDate: this.documentRetrieve.transactionDate,
          psAcctNo: this.documentRetrieve.acctNo,
          accountName: this.documentRetrieve.acctName,
          psAcctType: this.documentRetrieve.acctType,
          //pnGlobalView: 'N',
          ServiceId: this.serviceId,
          RoleId: this.assignRole.roleId,
          MenuId:  this.menuId,
          psStatus: this.documentRetrieve.status,
          referenceNo : this.documentRetrieve.referenceNo,
          amount: this.documentRetrieve.amount,
          psCcyCode: this.documentRetrieve.ccyCode,
          pnDeptId: this.documentRetrieve.processingDeptId,
          psBranchNo: this.documentRetrieve.branchNo,
        }
        //this.assignRole.roleId
    console.log('Document Retrieval param search', val);

    let url = 'DocumentRetrieval/GetAllSearchHistory';

    this._GeneralService.post(val, url)
    .subscribe((response: any) => {
      this.loadPage = false;
      //console.log("response doc retrieval search " + response._response);
      console.log("Response " + response._response);
...
clearItem()
{
  this.documentRetrieve.transactionDate = '';
  this.documentRetrieve.branchNo = '';
  this.documentRetrieve.processingDeptId = null;

  // this.assignRole.roleId,
  // this.menuId,
  this.documentRetrieve.status = '';
  this.documentRetrieve.referenceNo = '';
  this.documentRetrieve.acctNo = '';
  this.documentRetrieve.acctName = '';
  this.documentRetrieve.amount = '';
  this.documentRetrieve.acctType = '';
  this.documentRetrieve.ccyCode = '';
}
