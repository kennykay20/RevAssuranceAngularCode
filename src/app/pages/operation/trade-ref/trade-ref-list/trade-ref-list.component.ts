...
import { TradeRefDetailsComponent } from '../trade-ref-details/trade-ref-details.component';
...
@Component({
  selector: 'app-trade-ref-list',
  templateUrl: './trade-ref-list.component.html',
  styleUrls: ['./trade-ref-list.component.scss']
})
export class TradeRefListComponent implements OnInit {
 //  @ViewChild(DatatableComponent) table: DatatableComponent;
  ...
  rows: OprTradeReference[];
  rowSearch: OprTradeReference[];
  columns = [];
  temp:OprTradeReference[];
  current_page = 1;

  tradeRequestSearch: OprTradeReference;
...
userDetails: UserDetails;
  ...
  ngOnInit() 
  {
     ...
     this.tradeRequestSearch = new OprTradeReference();
...
clearItem()
{
  this.tradeRequestSearch.transactionDate = "";
  this.tradeRequestSearch.ccyCode = '';
  this.tradeRequestSearch.status = '';
  this.tradeRequestSearch.acctName = '';
  this.tradeRequestSearch.acctNo = '';
  this.tradeRequestSearch.acctStatus = '';
  this.tradeRequestSearch.acctType = '';
  this.tradeRequestSearch.processingDeptId = '',
  this.tradeRequestSearch.branchNo = '',
  this.tradeRequestSearch.availBal = '',
  this.tradeRequestSearch.referenceNo = '',
  this.tradeRequestSearch.amount =''
}

formatAmount(event){
  this.tradeRequestSearch.amount =  this._GeneralService.formatMoney(event.target.value)
}
...
fromDate(events)
{
  var dateCon = this._GeneralService.dateconvertion(events.target.value);
    ...
    this.tradeRequestSearch.transactionDate = dateCon;
    console.log("DateCon ", this.tradeRequestSearch.transactionDate);
}
...
searchHistory()
{
  ...
  let val = {
    transactionDate: this.tradeRequestSearch.transactionDate,
    psAcctNo: this.tradeRequestSearch.acctNo,
    psAcctType : this.tradeRequestSearch.acctType,
    accountName: this.tradeRequestSearch.acctName,
    //pnGlobalView: 'N',
    psCcyCode: this.tradeRequestSearch.ccyCode,
    RoleId: this.assignRole.roleId,
    MenuId:  this.menuId,
    referenceNo: this.tradeRequestSearch.referenceNo,
    serviceId: this.serviceId,
    Amount: this.tradeRequestSearch.amount,
    availBal: this.tradeRequestSearch.availBal,
    acctStatus: this.tradeRequestSearch.acctStatus,
    pnDeptId: this.tradeRequestSearch.processingDeptId,
    psBranchNo: this.tradeRequestSearch.branchNo
  }

  console.log('Trade param search', val);

  let url = 'TradeReference/GetAllSearchHistory';

  this._GeneralService.post(val, url)
  .subscribe((response: any) => {
    console.log("response trade search " + response._response);
  ...
  updateFilter(event, value: any) {
    ...
    if (value === 11) {
      ...
      const temp = this.temp.filter(function(d) {
        return d.userProcessingDept.toLowerCase().indexOf(val) !== -1 || !val;
      });
