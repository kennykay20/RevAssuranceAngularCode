...
@Component({
  selector: 'app-overdraft-req-details',
  templateUrl: './overdraft-req-details.component.html',
  styleUrls: ['./overdraft-req-details.component.scss']
})

export class OverdraftReqDetailsComponent implements OnInit {
  ...
  constructor(public appSettings: AppSettings,
    ...
    public dialogRef: MatDialogRef<OverdraftReqDetailsComponent>,
  ...
  reformInstrumentForm() {
    ...
    this.instrumentForm = new OverDraft();
  ...
  getById(recordId): void {

    console.log('OverDraft getById recordId:', recordId);

    this.loadPage = true;

    let track = 0;
    let url = 'OverDraft/GetById';
    ...
    this._GeneralService.post(val, url)
      ...
      .subscribe(
        (data: any) => {
          ...
          this.instrumentForm.approvedLimit = this.instrumentForm.approvedLimit != null ? this._GeneralService.formatMoney(this.instrumentForm.approvedLimit) : this.instrumentForm.approvedLimit;
          ...
          this.instrumentForm.startDate = this.instrumentForm.startDate != null ? this._GeneralService.dateconvertion(this.instrumentForm.startDate) : this.instrumentForm.startDate;
          this.instrumentForm.endDate = this.instrumentForm.endDate != null ? this._GeneralService.dateconvertion(this.instrumentForm.endDate) : this.instrumentForm.endDate;
          this.instrumentForm.newExpiryDate = this.instrumentForm.newExpiryDate != null ? this._GeneralService.dateconvertion(this.instrumentForm.newExpiryDate) : this.instrumentForm.newExpiryDate;
  ...
  add(values: Object): void {
    ...
    if(this.instrumentForm.acctType == null)
    {
      ...
      swal('', 'Account Type is required', 'error');
      return;
    }
    if(this.instrumentForm.acctNo == null || this.instrumentForm.acctNo == "")
    {
      swal('', 'Account Number is required', 'error');
      return;
    }
    // if(this.instrumentForm.acctName == null)
    // {
    //   swal('', 'Account Name is required', 'error');
    //   return;
    // }
    if(this.instrumentForm.ccyCode == null || this.instrumentForm.ccyCode == "")
    {
      this.instrumentForm.ccyCode = "";
      swal('', 'Currency is required', 'error');
      return;
    }
    if(this.instrumentForm.processingDeptId == null || this.instrumentForm.processingDeptId == "")
    {
      this.instrumentForm.processingDeptId = "";
      swal('', 'Select Department', 'error');
      return;
    }
    if(this.instrumentForm.rsmId == null)
    {
      
      swal('', 'Select Rsm', 'error');
      return;
    }
    if(this.instrumentForm.acctSic == null || this.instrumentForm.acctSic == "")
    {
      this.instrumentForm.acctSic = "";
      swal('', 'Select Account Sector', 'error');
      return;
    }
    ...
    let val =
    {
      OprOverDraft: this.instrumentForm,
  ...
  validateAcct(event) {
    ...
    console.log('this.instrumentForm.acctType', this.instrumentForm.acctType)
    // if (this.instrumentForm.acctType === '' || this.instrumentForm.acctType === undefined) {
    //   Swal('', "Select Acct Type", 'error');
    //   return;
    // }
  ...
  endDate1(): void {
    ...
    this._GeneralService.post(val, url).subscribe(
      (data: any) => {
        ...
        this.instrumentForm.newExpiryDate = data.expiryDate
  ...
  */
    ...
    //let url = 'ServiceCharge/CalCulateChargeOverDraft';
    ...
    this._GeneralService.post(values, url).subscribe(
      (data: any) => {
        ...
        this.instrumentForm.rsmId = data.instrumentAcctDetails.sRsmIdString == null ? 0 : data.instrumentAcctDetails.sRsmIdString;

        this.instrumentForm.odOption = data.instrumentAcctDetails.sOdOptionValue;
        this.instrumentForm.odExpiryDate = data.instrumentAcctDetails.sOdExpiryDate
        this.instrumentForm.drIntRate = data.instrumentAcctDetails.nDrIntRate
        this.instrumentForm.unapprovedOdRate = data.instrumentAcctDetails.nUnapprovedOdRate
        this.instrumentForm.acctCustNo = data.instrumentAcctDetails.custId
        this.instrumentForm.odlimit = this._GeneralService.formatMoney(data.instrumentAcctDetails.nOdLimit);
        this.instrumentForm.availBal = data.instrumentAcctDetails.nBalance;
  ...
  getTempCont(): void {
    ...
    this._GeneralService.post(val, url).subscribe(
      (data: any) => {
        ...
        console.log('getTempCont data', data);
        this.loadPage = false;
        this.templateContent = data.template.templateContent;
        console.log('viewTemp data', this.templateContent);
  ...
  openTemp(data: any): void {
    ...
    if (this.templateContent != null) {
      console.log('openTemp', data);
      let dialogRef = this.dialog.open(TemplateComponent, {
        data: { data: this.templateContent, serviceId: this.serviceId }
      });
