...
@Component({
  selector: 'app-bid-performance-list',
  templateUrl: './bid-performance-list.component.html',
  styleUrls: ['./bid-performance-list.component.scss']
})
export class BidPerformanceListComponent implements OnInit {
...
summernote1 = "dshsdhdhshdsjdhsdj";
  ...
  ngOnInit() 
  {
    ...
    //  $("#summernote").summernote("code", this.summernote1);
      ...
      this.bidPerformance = new InstrumentForm();
...
searchHistory()
{
  ...
  let val = 
        {
          transactionDate: this.bidPerformance.transactionDate,
          acctNo: this.bidPerformance.acctNo,
          accountName: this.bidPerformance.acctName,
          acctType: this.bidPerformance.acctType,
          pnGlobalView: 'N',
          serviceId: this.serviceId,
          RoleId: this.assignRole.roleId,
          MenuId:  this.menuId,
          psStatus: this.bidPerformance.status,
          referenceNo : this.bidPerformance.referenceNo,
          amount: this.bidPerformance.amount,
          CcyCode: this.bidPerformance.ccyCode,
          pnDeptId: this.bidPerformance.processingDeptId,
          psBranchNo: this.bidPerformance.branchNo,
        }
        //this.assignRole.roleId
    console.log('Bank Guarantee param search', val);

    let url = 'Instrument/GetAllSearchHistory';

    this._GeneralService.post(val, url)
    .subscribe((response: any) => {
      console.log("response bank guarantee search " + response._response);
      console.log("Response " + response);
      this.rowSearch = response._response;
      this.loadPage = false;
      element.disabled = false;

      if(response._response.length > 0)
      {
          this.rows  =  response._response.slice(0, this.pageLmit);
          this.changePage(this.current_page);
      }
      //this.changePage(this.current_page);
    }, error => {
...
clearItem(){
  this.bidPerformance.transactionDate = '';
  this.bidPerformance.branchNo = '';
  this.bidPerformance.processingDeptId = null;

  // this.assignRole.roleId,
  // this.menuId,
  this.bidPerformance.status = '';
  this.bidPerformance.referenceNo = '';
  this.bidPerformance.acctNo = '';
  this.bidPerformance.acctName = '';
  this.bidPerformance.amount = '';
  this.bidPerformance.acctType = '';
  this.bidPerformance.ccyCode = '';
}
...
fromDate(events)
  {
    ...
    this.bidPerformance.transactionDate = dateCon;
...
formAmt(event){
  ...
  this.bidPerformance.amount = this._GeneralService.formatMoney(event.target.value);
...
keyPress(event: any) {
  ...
  View(action: any, record: InstrumentForm, createdBy: any): void
   {
    ...
    let dialogRef = this.dialog.open(ApgDetailsComponent, {
      ...
      data: { actionName: action, record:  record, createdBy: createdBy, 
              ...
              serviceName: 'BID PERFORMANCE',  admService :  this.admService,
  ...
  swapTab(value){
    ...
    this.tabVlues = value;
    console.log("tabValues " + this.tabVlues);
  }
