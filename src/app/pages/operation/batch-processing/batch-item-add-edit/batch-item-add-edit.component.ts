...
@Component({
  selector: 'app-batch-item-add-edit',
  templateUrl: './batch-item-add-edit.component.html',
  styleUrls: ['./batch-item-add-edit.component.scss']
})

export class BatchItemAddEditComponent  implements OnInit {
...
constructor(public appSettings: AppSettings, 
              ...
              public dialogRef: MatDialogRef<BatchItemAddEditComponent>,
        ...
        )
      {
       ...
       // this.resetAction(data);
        this.iniTranCon();
        console.log('data details item; ', data);
...
.retryWhen((err) => {
  ...
  let  valAcct ;
   ...
   // this.instrumentForm.itbId = 0;
     let tem = this.instrumentForm;
      let val = 
      { 
            BatchItems: this.instrumentForm,
      ...
      let url = 'BatchControl/AddBatchItem';
  ...
  update(): void {
    ...
    element.disabled = true;
      let url = 'BatchControl/UpdateBatchItem';
  ...
  actionItem(action: any, record: any): void  //////
  {
   ...
   let dialogRef = this.dialog.open(BatchItemAddEditComponent, {
...
iniTranCon(){
  let hhh = new AdmTransactionConfiguration();
  this.admTransactionConfiguration = hhh;
}
  ...
  for(let i = 0; i <= this.chargeFormList.length; i++)
  {
      if(this.chargeFormList[i] !== undefined)
      {
        ...
        */
        ...
        cal =  Number(this.chargeFormList[i].nTaxRate) / 100 * Number(this.chargeFormList[i].equivChgAmount)
        this.chargeFormList[i].taxAmount = this._GeneralService.formatMoney(cal);
       // console.log(' this.chargeFormList[i].taxAmount: ',  this.chargeFormList[i].taxAmount);
