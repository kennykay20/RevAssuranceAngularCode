...
import { BatchControlItemDetailsComponent } from '../batch-control--item-details/batch-control--item-details.component';
import { BatchControl } from '../../../../model/BatchControl';
import { UserDetails } from '../../../../model/userDetails';
import * as XLSX from 'xlsx';
import { UploadBulkComponent } from '../upload-bulk/upload-bulk.component';
...
userDetails: UserDetails
  ...
  select(row)
  {
        console.log('selected row', this.selectedRec);
  ...
  selectAll() 
  {
    console.log("selectedAllTrans ", this.selectedAllTrans);
  ...
  View(action: any, record: any, createdBy: any): void
   {
    ...
    let dialogRef = this.dialog.open(BatchControlItemDetailsComponent, {
 ...
 removeBatches(): void {
  ...
  if(getSelected.length === 0) {
    ...
    Swal({
      ...
      title: '',
      text: 'Are you Sure you want to Remove the Selected Batch(es)?',
      type: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Yes',
      confirmButtonColor: this.btnConfirm, 
...
let data = [];
  ...
  for(let i = 1; i < 7; i++)
  {
   ...
   this._GeneralService.exportAsExcelFile1(data, 'BatchBulkPaymentUploadTemplateFormat');
 }

////====Uploads details below =====///
gotoUpload(action: any, record: any, createdBy: any): void
{

 let dialogRef = this.dialog.open(UploadBulkComponent, {
...
processFileUpload(): void 
{
  ...
  if(getSelected.length === 0) 
  {
    Swal('', 'Select the Batch you want to Process', 'warning');
    return;
  }
    Swal({

      title: '',
      text: 'Are you Sure you want to Load the Selected Batch(es) fully?',
      type: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Yes',
      confirmButtonColor: this.btnConfirm, 
...
processManual(): void 
{
  ...
  if(getSelected.length === 0) 
  {
    Swal('', 'Select the Batch(es) you want to Process', 'warning');
    return;
  }
    Swal({
      ...
      if (result.value) 
      {
        ...
        let url = 'BatchControl/ProcessAllItem';
