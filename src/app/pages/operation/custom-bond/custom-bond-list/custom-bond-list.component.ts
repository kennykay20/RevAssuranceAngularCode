...
import {MatDialog, MatDialogRef, MatDialogConfig, MAT_DIALOG_DATA, MatIcon} from '@angular/material';
...
@Component({
  selector: 'app-custom-bond-list',
  templateUrl: './custom-bond-list.component.html',
  styleUrls: ['./custom-bond-list.component.scss']
})
export class CustomBondListComponent implements OnInit {
...
summernote1 = "dshsdhdhshdsjdhsdj";
  ...
  ngOnInit() 
  {
    ...
    //  $("#summernote").summernote("code", this.summernote1);
      ...
      this.customBond = new InstrumentForm();
...
fromDate(events)
{
    ...
    this.customBond.transactionDate = dateCon;
...
searchHistory()
{
  this.loadPage = true;
  let track = 0;
  //alert("this.serviceId " + this.serviceId);
  let element = <HTMLInputElement> document.getElementById("btnSearchEnq");
  element.disabled = true;

  let val = 
        {
          transactionDate: this.customBond.transactionDate,
          acctNo: this.customBond.acctNo,
          accountName: this.customBond.acctName,
          acctType: this.customBond.acctType,
          pnGlobalView: 'N',
          serviceId: this.serviceId,
          RoleId: this.assignRole.roleId,
          MenuId:  this.menuId,
          psStatus: this.customBond.status,
          referenceNo : this.customBond.referenceNo,
          amount: this.customBond.amount,
          CcyCode: this.customBond.ccyCode,
          pnDeptId: this.customBond.processingDeptId,
          psBranchNo: this.customBond.branchNo
        }
        //this.assignRole.roleId
    console.log('Custom Bond param search', val);

    let url = 'Instrument/GetAllSearchHistory';

    this._GeneralService.post(val, url)
    .subscribe((response: any) => {
      console.log("response Custom search " + response._response);
...
clearItem()
{
  this.customBond.transactionDate = "";
  this.customBond.ccyCode = '';
  this.customBond.status = '';
  this.customBond.acctName = '';
  this.customBond.acctNo = '';
  this.customBond.acctStatus = '';
  this.customBond.acctType = '';
  this.customBond.processingDeptId = 0,
  this.customBond.branchNo = '',
  this.customBond.availBal = '',
  this.customBond.referenceNo = '',
  this.customBond.amount =''
}
  ...
  View(action: any, record: InstrumentForm, createdBy: any): void
   {
    ...
    let dialogRef = this.dialog.open(ApgDetailsComponent, {
      ...
      data: { actionName: action, record:  record, createdBy: createdBy, 
              ...
              serviceName: 'CUSTOM BOND',  admService :  this.admService,
