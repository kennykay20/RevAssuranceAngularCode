...
@Component({
  selector: 'app-reverse-trans-list',
  templateUrl: './reverse-trans-list.component.html',
  styleUrls: ['./reverse-trans-list.component.scss']
})
export class ReverseTransListComponent implements OnInit {
...
apiIsDown = GenModel.apiIsDown;
  ...
  */
  ...
  updateFilter1(event, value: any) {
    ...
    if (value === 8) {
      ...
      const temp = this.temp.filter(function(d) {
        return d.amount.toLowerCase().indexOf(replaceVal) !== -1 || !replaceVal;
      });
     ...
     setTimeout(() => {
      ...
      let dialogRef = this.dialog.open(PostTransDetailsComponent, {
        ...
        data: { actionName: actionName, record:  record, createdBy: 1, 
          chargeSetup: this.chargeSetup, userId: this.userId, itbId: record.itbId},
 ...
 keyPress(event: any) {
  ...
  reverseTrans(): void {
    ...
    let msgg = `Are you sure you want to reverse the selected ${getSelected.length} transaction(s)?`;
    ...
    Swal({
      ...
      if (result.value) {
        ...
        let url = 'ReverseTrans/ReverseTxn';
...
checkUnChecked(row: CbsTransaction) {
  ...
  fromDateTxn(event) {
    this.fromDate =  this._GeneralService.dateconvertion(event.target.value)
  }

  toDateTxn(event) {
    this.toDate =  this._GeneralService.dateconvertion(event.target.value)
  }

  retrieve(param: any):void{
    this.loadPage = true;
    let track = 0;
    let url = 'ReverseTrans/Retrieve';

      let val = 
        {
          userId: this.userDetails.userId,
          pdtCurrentDate: this.userDetails.bankingDate,
        ...
        console.log('retrieve param', val);
