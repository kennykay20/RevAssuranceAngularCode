...
@Component({
  selector: 'app-performance-bond-list',
  templateUrl: './performance-bond-list.component.html',
  styleUrls: ['./performance-bond-list.component.scss']
})
export class PerformanceBondListComponent implements OnInit {
 //  @ViewChild(DatatableComponent) table: DatatableComponent;
  ...
  performanceBond: InstrumentForm;
  columns = [];
  temp: InstrumentForm [];
...
summernote1 = "dshsdhdhshdsjdhsdj";
  ...
  ngOnInit() 
  {
    ...
    //  $("#summernote").summernote("code", this.summernote1);
      ...
      this.performanceBond = new InstrumentForm();
...
fromDate(events)
{
  var dateCon = this._GeneralService.dateconvertion(events.target.value);
    ...
    this.performanceBond.transactionDate = dateCon;
...
searchHistory()
{
  ...
  let val = 
        {
          transactionDate: this.performanceBond.transactionDate,
          acctNo: this.performanceBond.acctNo,
          accountName: this.performanceBond.acctName,
          acctType: this.performanceBond.acctType,
          //pnGlobalView: 'N',
          ServiceId: this.serviceId,
          RoleId: this.assignRole.roleId,
          MenuId:  this.menuId,
          psStatus: this.performanceBond.status,
          referenceNo : this.performanceBond.referenceNo,
          amount: this.performanceBond.amount,
          CcyCode: this.performanceBond.ccyCode,
          pnDeptId: this.performanceBond.processingDeptId,
          psBranchNo: this.performanceBond.branchNo
        }
        //this.assignRole.roleId
    console.log('Performance value param search', val);

    let url = 'Instrument/GetAllSearchHistory';

    this._GeneralService.post(val, url)
    .subscribe((response: any) => {
      console.log("response performance search " + response._response);
...
clearItem()
{
  this.performanceBond.transactionDate = "";
  this.performanceBond.ccyCode = '';
  this.performanceBond.status = '';
  this.performanceBond.acctName = '';
  this.performanceBond.acctNo = '';
  //this.performanceBond.acctStatus = '';
  this.performanceBond.acctType = '';
  this.performanceBond.processingDeptId = null,
  this.performanceBond.branchNo = '',
  this.performanceBond.availBal = '',
  this.performanceBond.referenceNo = ''
  ...
  View(action: any, record: InstrumentForm, createdBy: any): void
   {
    ...
    let dialogRef = this.dialog.open(ApgDetailsComponent, {
      ...
      data: { actionName: action, record:  record, createdBy: createdBy, 
        ...
        serviceName: 'PERFORMANCE BOND',  admService :  this.admService,
