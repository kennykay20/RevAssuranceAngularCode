...
import { PinResetDetailsComponent } from '../pin-reset-details/pin-reset-details.component';
...
@Component({
  selector: 'app-pin-reset-list',
  templateUrl: './pin-reset-list.component.html',
  styleUrls: ['./pin-reset-list.component.scss']
})
export class PinResetListComponent implements OnInit {
  ...
  rows: DocRetrieval[];
  rowSearch: DocRetrieval[];
  columns = [];
  temp: DocRetrieval[];
  current_page = 1;

  pinResetBound: DocRetrieval;
  ...
  pinTransactionHistory: DocRetrieval;
  currentDate: any = '';
  pinTransactionDate = '';
  ...
  ngOnInit() {
    ...
    this.pinResetBound = new DocRetrieval();
...
.retryWhen((err) => {
  ...
  loadAll(): void { 
    ...
    this._GeneralService.post(null, url).subscribe(
      (data: any) => 
      {
        ...
        this.pinCurrencies = data.currencies;
        this.admServiceList =  data.admService;
        this.branches = data.branch;
  
        console.log("currencies " + this.pinCurrencies);
        this.loadPage = false;
  ...
  searchHistory()
  {
    ...
    let element = <HTMLInputElement> document.getElementById("btnSearchPin");
    element.disabled = true;

    let val = {
      transactionDate: this.pinResetBound.transactionDate,
      psAcctNo: this.pinResetBound.acctNo,
      psAcctType : this.pinResetBound.acctType,
      accountName: this.pinResetBound.acctName,
      // pnGlobalView: 'N',
      psCcyCode: this.pinResetBound.ccyCode,
      RoleId: this.assignRole.roleId,
      MenuId:  this.menuId,
      referenceNo: this.pinResetBound.referenceNo,
      serviceId: this.serviceId,
      // availBal: this.pinResetBound.availBal,
      acctStatus: this.pinResetBound.acctStatus,
      pnDeptId: this.pinResetBound.processingDeptId,
      psBranchNo: this.pinResetBound.branchNo,
      pinIds:     this.pinResetBound.pinIds
    }

    console.log('Pin Reset param search', val);

    let url = 'PinReset/GetAllPinSearchHistory';

    this._GeneralService.post(val, url)
    .subscribe((response: any) => {
      console.log("response Pin Reset search " + response._response);
  ...
  clearItem()
  {
     this.pinResetBound.transactionDate = ''
     this.pinResetBound.acctNo = '',
     this.pinResetBound.acctType = '',
     this.pinResetBound.acctName = '',
      
      this.pinResetBound.ccyCode = '',
       
      this.pinResetBound.referenceNo = '',
      // this.pinResetBound.availBal = '',
      this.pinResetBound.acctStatus = '',
       this.pinResetBound.processingDeptId = '',
      this.pinResetBound.branchNo = '',
      this.pinResetBound.pinIds = ''
  ...
  View(action: any, record: any, createdBy: any): void {
    let dialogRef = this.dialog.open(PinResetDetailsComponent, {
      ...
      data: {
        ...
        serviceName: 'PIN RESET', admService: this.admService,
        oprPinItems: this.oprPinItems
      },
  ...
  fromDate(events)
  {
    ...
    this.pinResetBound.transactionDate = dateCon;
