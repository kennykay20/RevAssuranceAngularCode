...
import { StopChqDetailsComponent } from '../stop-chq-details/stop-chq-details.component';


declare var $;

@Component({
  selector: 'app-stop-chq-list',
  templateUrl: './stop-chq-list.component.html',
  styleUrls: ['./stop-chq-list.component.scss']
})
export class StopChqListComponent implements OnInit {
  ...
  rowSearch: StopChequeForm[];

  stopChequeRef: StopChequeForm;
  selectAllText: any;
...
tab2 = '';
  ...
  ngOnInit() 
  {
     //console.log()
      ...
      this.stopChequeRef = new StopChequeForm();
...
fromDate(events)
{
  var dateCon = this._GeneralService.dateconvertion(events.target.value);
    ...
    this.stopChequeRef.transactionDate = dateCon;
...
searchHistory()
{
  ...
  let val = 
        {
          transactionDate: this.stopChequeRef.transactionDate,
          psAcctNo: this.stopChequeRef.acctNo,
          accountName: this.stopChequeRef.acctName,
          psAcctType: this.stopChequeRef.acctType,
          //pnGlobalView: 'N',
          ServiceId: this.serviceId,
          RoleId: this.assignRole.roleId,
          MenuId:  this.menuId,
          serviceStatus: this.stopChequeRef.serviceStatus,
          referenceNo : this.stopChequeRef.referenceNo,
          accountStatus: this.stopChequeRef.acctStatus,
          psCcyCode: this.stopChequeRef.ccyCode,
          pnDeptId: this.stopChequeRef.processingDeptId,
          psBranchNo: this.stopChequeRef.branchNo,
        }
        //this.assignRole.roleId
    console.log('Stop Cheque param search', val);

    let url = 'StopCheque/GetAllSearchHistory';

    this._GeneralService.post(val, url)
    .subscribe((response: any) => {
      console.log("response stop cheque search " + response._response.origBranchName);
...
clearItem()
{
  this.stopChequeRef.transactionDate = '';
  this.stopChequeRef.acctName = '';
  this.stopChequeRef.acctNo = '';
  this.stopChequeRef.acctType = '';
  this.stopChequeRef.ccyCode = '';
  this.stopChequeRef.branchNo = '';
  this.stopChequeRef.processingDeptId = null;
  this.stopChequeRef.acctStatus = '';
  this.stopChequeRef.serviceStatus = '';
  ...
  View(action: any, record: any, createdBy: any): void
   {
    let dialogRef = this.dialog.open(StopChqDetailsComponent, {
      ...
      data: { actionName: action, record:  record, createdBy: createdBy, 
        ...
        serviceName: 'STOP Cheque',  admService :  this.admService
      },
