...
@Component({
  selector: 'app-all-pub-ann',
    templateUrl: './all-pub-ann.component.html',
    styleUrls: ['./all-pub-ann.component.scss']
})
export class AllPubAnnComponent implements OnInit {
 ...
 //   this.ItemsPerPage = this._GeneralService.ItemsPerPage;
  ...
  load(): void {
    ...
    this._GeneralService.getT(token, 'auth/all-published-announcement?token=')
    ...
    .retryWhen((err) => {
      ...
      (data: any) => {
        ...
        let rec = new List<any>(data).Where(c => c.status == 'Published').ToArray();

        this.loadPage = false;
        this.temp = rec;
        this.rows = rec;
        this.rowsMobile = rec.slice(0, this.pageLmit);
        this.rowsMobileTem = rec;
   ...
   View(row: any): void {
    ...
    this._localStorageService.set(this.redirectUrl, './ano/allPubAnn'); 
