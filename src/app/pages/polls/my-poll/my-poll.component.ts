...
export class MyPollComponent implements OnInit {
 @ViewChild(DatatableComponent) table: DatatableComponent;
  ...
  loadMyPoll(): void {
    ...
    this._GeneralService.getT(token, 'auth/my-polls?token=')
   ...
   View(row: any): void {
        ...
        this._localStorageService.set(this.redirectUrl, './pol/myPoll'); 
        
        this.router.navigate(['./pol/view']);
   }
  ...
  updateFilter(event) {
    ...
    if (this.columnId === 4) {
      ...
      const temp = this.temp.filter(function(d) {
        return d.maker_name.toLowerCase().indexOf(val) !== -1 || !val;
      });
