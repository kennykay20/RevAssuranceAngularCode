...
import swal from 'sweetalert2';
import {DocumentChgDetailsComponent} from '../document-chg-details/document-chg-details.component';

declare var $;

@Component({
  selector: 'app-document-chg-list',
  templateUrl: './document-chg-list.component.html',
  styleUrls: ['./document-chg-list.component.scss']
})
export class DocumentChgListComponent implements OnInit {
...
menuId: any;
  ...
  updateFilter(event, value: any) {
    ...
    if (value === 2) {
      const val = event.target.value.toLowerCase();
      console.log('this.temp description ', this.temp);
      const temp = this.temp.filter(function(d) {
        return d.description.toLowerCase().indexOf(val) !== -1 || !val;
      });

      console.log('this.rows desc ', this.rows);
      this.rows = temp;
    ...
    if (value === 3) {
      const val = event.target.value.toLowerCase();
      console.log('this.temp charge ', this.temp);
      const temp = this.temp.filter(function(d) {
        return d.chgAmount;
      });
      console.log('this.rows charge ', this.rows);
      this.rows = temp;
  ...
  openDialog(action: any, record: any): void
   {
    ...
    let dialogRef = this.dialog.open(DocumentChgDetailsComponent, {
