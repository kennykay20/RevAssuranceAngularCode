...
import swal from 'sweetalert2';
import { TemplateSetUpDetailsComponent } from '../template-set-up-details/template-set-up-details.component';

declare var $;

@Component({
  selector: 'app-template-set-up-list',
  templateUrl: './template-set-up-list.component.html',
  styleUrls: ['./template-set-up-list.component.scss']
})
export class TemplateSetUpListComponent implements OnInit {
  ...
  load(): void {
    ...
    .retryWhen((err) => {
      ...
      (data: any) => {
        ...
        this.temp = data;
        this.rows = data;
        console.log("this.rows template", this.rows);
  ...
  updateFilter(event, value: any) {
    ...
    if (value === 1) {
      ...
      const temp = this.temp.filter(function(d) {
        return d.templateCode.toLowerCase().indexOf(val) !== -1 || !val;
      });
    ...
    if (value === 3) {
      ...
      const temp = this.temp.filter(function(d) {
        console.log("d ", d.serviceDescription);
  ...
  openDialog(action: any, record: any): void
   {
    ...
    let dialogRef = this.dialog.open(TemplateSetUpDetailsComponent, {
