...
import swal from 'sweetalert2';
import { ServiceRefDetailsComponent } from '../service-ref-details/service-ref-details.component';

declare var $;

@Component({
  selector: 'app-service-ref-list',
    templateUrl: './service-ref-list.component.html',
    styleUrls: ['./service-ref-list.component.scss']
})
export class ServiceRefListComponent implements OnInit {
...
//fkk:boolean = true;
  ...
  loaderTime = GenModel.LoderTimer;
...
origMobile = [];
  ...
  updateFilter(event, value: any) {
    ...
    if (value === 2) {
      ...
      const temp = this.temp.filter(function(d) {
        return d.referenceFormat.toLowerCase().indexOf(val) !== -1 || !val;
      });
  ...
  openDialog(action: any, record: any): void
   {
    ...
    let dialogRef = this.dialog.open(ServiceRefDetailsComponent, {
