...
import swal from 'sweetalert2';
import { ChequeProductDetailsComponent } from '../cheque-product-details/cheque-product-details.component';

declare var $;


@Component({
  selector: 'app-cheque-product-list',
    templateUrl: './cheque-product-list.component.html',
    styleUrls: ['./cheque-product-list.component.scss']
})
export class ChequeProductListComponent implements OnInit {
  ...
  updateFilter(event, value: any) {
    ...
    if (value === 1) {
      ...
      const temp = this.temp.filter(function(d) {
        return d.chqProductCode.toLowerCase().indexOf(val) !== -1 || !val;
      });
    ...
    if (value === 2) {
      ...
      const temp = this.temp.filter(function(d) {
        return d.chqProductDescr.toLowerCase().indexOf(val) !== -1 || !val;
      });
  ...
  openDialog(action: any, record: any): void
   {
    ...
    let dialogRef = this.dialog.open(ChequeProductDetailsComponent, {
