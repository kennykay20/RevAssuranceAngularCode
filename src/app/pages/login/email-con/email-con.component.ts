...
@Component({
  selector: 'app-email-con',
  templateUrl: './email-con.component.html',
  styleUrls: ['./email-con.component.scss']
})
export class EmailConComponent implements OnInit {
...
displayloaderLogin = false;
  ...
  ngOnInit1() {
      ...
      this.route.queryParams.subscribe(params => {
        this.tokenEmailCon = params.I;
       
         console.log('**changePwd this.tokenEmailCon: ', this.tokenEmailCon);
  ...
  public onSubmit(values: any): any {
    ...
    // if (this._GeneralService.isOnline() === false)
    //  {
    ...
    this.btnChangPwd = 'Wait Changing Password...';
    
    this.displayloader = true;

  let Body = {
                  password: values.password,
                  token: this.tokenEmailCon
              };

         console.log('Body change Body: ', Body);

      this._GeneralService.post(Body, `password/recover`)
      ...
      subscribe(
        (data: any) => {
              ...
              this.displayloaderLogin = true;
              swal('', data.message, 'success');
        },
        (error: any) => 
        {     
            //console.log('onSubmit change Pssword error', error);
              this.btnChangPwd = 'SUBMIT';
              this.loginSucc = false;
              this.displayloader =  false;
